package vip.ruoyun.skin.butler;
...
import vip.ruoyun.skin.butler.core.SkinCache;

public class SkinButler {

    private static class SkinButlerHolder {

        private static SkinButler INSTANCE = new SkinButler();
    }

    public static SkinButler instance() {
        return SkinButlerHolder.INSTANCE;
    }


    public static void init(Application application) {
        SkinButler skinButler = instance();
        skinButler.setup(application);
    }
...
//            addAssetPath.setAccessible(true);
            ...
            // 创建加载外部的皮肤包(app.skin)文件Resources（注：依然是本应用加载）
        ...
        } catch (Exception e) { // 发生异常，预判：通过skinPath获取skinPackageName失败！
//            e.printStackTrace();
            isDefaultSkin = true;
        }
    }
    ...
    private int getSkinResourceIds(int resourceId) {
        ...
        //TODO 缓存是否有资源，如果没有的话，不用判断直接
//        skinResources.getResourceName(resourceId);
